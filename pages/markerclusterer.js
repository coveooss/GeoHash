...
function MarkerClusterer(map, opt_markers, opt_options) {
  ...
  this.noCalc_ = options["styles"] || false;
...
MarkerClusterer.prototype.calculator_ = function (markers, numStyles) {
  ...
  //if (this.noCalc_){
  //We do have to take the total from each marker
  for (var i = 0, marker; (marker = markers[i]); i++) {
    if (marker.total) {
      count += marker.total;
...
MarkerClusterer.prototype.setReady_ = function (ready) {
  ...
  if (this.ready_) {
    ...
    var that = this;
    this.zoom_changed_listener = google.maps.event.addListener(
      this.map_,
      "zoom_changed",
      function () {
    ...
    this.idle_listener = google.maps.event.addListener(
      this.map_,
      "idle",
      function () {
        //COVEO, we do not want this, because that would cause to show points first after a zoom
        // and those will be replaced, very bad effect.
        //that.redraw();
      }
    );
...
MarkerClusterer.prototype.updateClusters = function () {
  //this.ready_ = true;
  this.redraw();
};
...
Cluster.prototype.addMarker = function (marker) {
  ...
  var len = count; //COVEO this.markers_.length;
...
Cluster.prototype.getBounds = function () {
  ...
  var markers = this.getMarkers();
  for (var i = 0, marker; (marker = markers[i]); i++) {
    if (marker.rectbounds) {
      bounds.extend(marker.rectbounds.getBounds().getNorthEast());
      bounds.extend(marker.rectbounds.getBounds().getSouthWest());
    } else {
      bounds.extend(marker.getPosition());
    }
  }
 ...
 */
Cluster.prototype.updateIcon = function (count) {
...
MarkerClusterer.prototype["updateClusters"] =
  MarkerClusterer.prototype.updateClusters;
